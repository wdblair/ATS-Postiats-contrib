...
//
end // end of [patsolve_smt_cmdline]

(* ****** ****** *)
//
extern fun patsolve_smt_help(): void
extern fun patsolve_smt_input(): void
extern fun patsolve_smt_gitem(string): void
extern fun patsolve_smt_input_arg(string): void
//
extern fun patsolve_smt_argend((*void*)): void
//
extern fun patsolve_smt_commarglst_finalize(): void
//
(* ****** ****** *)
...
(
  stdout_ref
, "patsolve_smt_commarglst: process_arg: x = ", x
) (* end of [val] *)
*)
//
in
...
case+ x of
//
| CAhelp _ => patsolve_smt_help ()
//
| CAinput _ => patsolve_smt_input ()
//
| CAgitem(str) => patsolve_smt_gitem(str)
//
(*
| CAoutput(str) => fprint! (out, "CAoutput(", str, ")")
| CAscript(str) => fprint! (out, "CAscript(", str, ")")
*)
| CAargend() => patsolve_smt_argend ()
//
| _ (*rest-of-CA*) => ()
//
...
implement
patsolve_smt_commarglst
  (xs) = let
(*
val () = println! ("patsolve_smt_commarglst")
*)
in
//
case+ xs of
| ~list_vt_cons
    ...
    val () = process_arg(x)
  in
    patsolve_smt_commarglst (xs)
  end // end of [list_vt_cons]
//
| ~list_vt_nil
    ((*void*)) => patsolve_smt_commarglst_finalize ()
  // end of [list_vt_nil]
//
end // end of [patsolve_smt_commarglst]
...
//
} (* end of [patsolve_smt_input] *)

(* ****** ****** *)

implement
patsolve_smt_gitem(arg) = let
//
val () =
prerrln!
  ("patsolve_smt_gitem: arg = ", arg)
//
...
| _ when input() =>
  {
    val () = patsolve_smt_input_arg(arg)
    val () = !the_state.ninput := !the_state.ninput+1
  }
| _ (*unrecognized*) => ()
//
end (* end of [patsolve_smt_gitem] *)

(* ****** ****** *)

implement
patsolve_smt_input_arg
  (path) = let
//
val
opt =
...
(*
    ...
    fprint! (
      stdout_ref
    , "patsolve_smt_input_arg: c3t0 =\n"
    ) (* end of [fprint] *)
    val () = fpprint_c3nstr(stdout_ref, c3t0)
    val () = fprint_newline (stdout_ref)
*)
//
    val ((*void*)) = c3nstr_smt_solve(c3t0)
//
  } (* end of [Some_vt] *)
//
| ~None_vt((*void*)) =>
...
//
end // end of [patsolve_smt_input_arg]
...
| _ (*rest*) => ((*ignored*))
//
end (* end of [patsolve_smt_argend] *)

(* ****** ****** *)

implement
patsolve_smt_commarglst_finalize
  ((*void*)) =
{
  val n0 = !the_state.fopen_inp
  val f0 = !the_state.inpfil_ref
  val () = if n0 > 0 then fileref_close(f0)
} (* end of [patsolve_smt_commarglst_finalize] *)

(* ****** ****** *)

(* end of [patsolve_smt_commarg.dats] *)
